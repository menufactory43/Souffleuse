...
- **Déployer le site (recette rapide)** : depuis `website/`, lancer `npx --yes vercel@latest --prod --yes`. Le binaire `vercel` n'est **pas** dans le PATH (d'où `npx`). Le projet est déjà lié (`website/.vercel/project.json` → projet `website`, org `family-flow-ios` / `team_cy8eMZTCugrYiA8Fci39vydt`) et l'auth est en cache (`~/Library/Application Support/com.vercel.cli/auth.json`) → **aucun login interactif**. Le deploy s'aliase automatiquement sur `souffleuse.app`. Vérifier le live ensuite (le push git ne déploie pas) : `curl -s "https://souffleuse.app/?cb=$(date +%s)" | rg -c data-version` (attendu 3) et l'email `contact@souffleuse.app`. Penser à re-déployer **après** le commit : un `vercel --prod` lancé avant de committer/sauver les fichiers met en ligne l'ancienne source.
- **⚠️ Compteur de téléchargements — NE PAS polluer en testant.** Le bouton public `/Souffleuse.dmg` passe par l'edge function `/api/download` qui **incrémente les compteurs KV** (`dl:total`, `dl:site|sparkle`, série journalière, et `dl:ver:{version}` lu dans l'appcast) avant de rediriger (302) vers le fichier réel. **Pour tout test/dev, télécharger le binaire via le chemin BRUT `https://souffleuse.app/dl/Souffleuse.dmg`** (fichier statique, **non compté**) — jamais via `/Souffleuse.dmg`. Idem pour vérifier le DMG en local : `website/dl/Souffleuse.dmg`. Lecture des stats : `GET /api/stats` (totaux + `?day=` + `?versions=1`). Avant lancement, **reset des compteurs KV** (les chiffres actuels incluent des tests).
...
- **Inférence** :
  ...
  - **MLX** (`MLX`/`MLXLLM`/`MLXLMCommon`) — **supprimé du package le 14/06/2026** : les 8 cibles dev mortes (dont `SouffleuseBench`, `SouffleuseCoherence`, `SouffleuseEnrichmentBench`, `SouffleuseTTFTBench`) constituaient les seuls consommateurs de `mlx-swift-examples`. La dépendance SPM n'existe plus dans `Package.swift` ni `Package.resolved`.
...
`SouffleuseAXProbe`, `SouffleuseContextProbe`, `SouffleuseLlamaProbe`, `SouffleuseOCRAblation`, `SouffleuseMidwordEval`, `SouffleuseRecallEval`, `SouffleuseInjectionEval`, `SouffleuseTranslateBench`, `SouffleuseReplay`, `SouffleuseCorpusSeed`. (Benches `print`-heavy → hors `SHIPPING_DIRS` de `audit.sh`.) Les 8 cibles mortes/MLX (`SouffleuseBench`, `SouffleuseCoherence`, `SouffleuseEnrichmentBench`, `SouffleuseTTFTBench`, `SouffleuseCorpusEval`, `SouffleuseBeamEval`, `SouffleusePersonalizationEval`, `SouffleuseMaxWordsEval`) ont été supprimées le 14/06/2026.
